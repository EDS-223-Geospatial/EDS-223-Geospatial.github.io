...
- [ ] **read the package's vignette, if available** - these are often linked on CRAN under the *Documents* sections (e.g. see [`{dplyr}` on CRAN](https://cran.r-project.org/web/packages/dplyr/index.html) or `help("dplyr")`) or can be found through the command `vignette(package = "package-name")` and `vignette("vignette-name")`
