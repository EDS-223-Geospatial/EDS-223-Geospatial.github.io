...
-   consider folding chunks for API and CRS transformation so users can focus on the plotting code

-   add an enhanced ggplot point map - colored/different basemap, subtitle, etc.

-   integrate plot_sf and tidyterra too?
...
Visualizing vector data, raster data, or both overlaid can be accomplished in various ways. This guide provides explanations for which are recommended to get the most out of your visualizations alongisde examples of code.
...
Use the `gbif` API to download species observations for polar bears from the [Global Biodiversity Information Facility](https://www.gbif.org/). Spatial data will not always have the critical spatial metadata associated with it, so you may have to manually assign it using spatial package functions before plotting.
...
Start with the basics: plot the point data using the native R function `plot()`.
...
#TODO: Add IUCN citation for data
...
Check that the CRS matches for both spatial objects before plotting or performing spatial operations. 

```{r}
st_crs(pb_habitat_poly) == st_crs(pb_spatial)
```
...
ggplot(data = canada_greenland) +
  ...
  coord_sf(
    xlim = st_bbox(canada_greenland)[c("xmin", "xmax")],
...
```{r}
fp_ice = "course-materials/resources/data/Arctic_ice/N_197812_concentration_v3.0.tif"
arctic_ice <- terra::rast(here(fp_ice))

terra::plot(arctic_ice,
            main = "Arctic Sea Ice Concentration")
```

Make a histogram of the raster values using the base R `hist` to better understand the numerical data you're visualizing:
