...
Refer to [Chapter 4](https://r.geocompx.org/spatial-operations.html) and [Chapter 5](https://r.geocompx.org/geometry-operations) of *Geocomputation with R* to get you
started.
:::
...
- **iNaturalist Research-grade Observations, 2020-2024** ([Access](https://github.com/ropensci/rinat))
...
sb_protected_areas <- sf::read_sf(here::here("course-materials", "data", "week3-discussion", "cpad_super_units_sb.shp")) %>% 
  sf::st_transform("ESRI:102009")
sb_city_boundaries <- sf::read_sf(here::here("course-materials", "data", "week3-discussion", "sb_city_boundaries_2003.shp")) %>%
  sf::st_transform("ESRI:102009")
sb_county_boundary <- sf::read_sf(here::here("course-materials", "data", "week3-discussion", "sb_county_boundary_2020.shp")) %>%
  sf::st_transform("ESRI:102009")
aves <- sf::read_sf(here::here("course-materials", "data", "week3-discussion", "aves_observations_2020_2024.shp")) %>%
  sf::st_transform("ESRI:102009")
```
...
sb_protected_areas <- sf::read_sf(here::here("data", "week3-discussion", "cpad_super_units_sb.shp")) %>% 
  sf::st_transform("ESRI:102009")
sb_city_boundaries <- sf::read_sf(here::here("data", "week3-discussion", "sb_city_boundaries_2003.shp")) %>%
  sf::st_transform("ESRI:102009")
sb_county_boundary <- sf::read_sf(here::here("data", "week3-discussion", "sb_county_boundary_2020.shp")) %>%
  sf::st_transform("ESRI:102009")
aves <- sf::read_sf(here::here("data", "week3-discussion", "aves_observations_2020_2024.shp")) %>%
  sf::st_transform("ESRI:102009")
```
...
aves_PA_join <- sf::st_join(aves, sb_protected_areas)
...
sf::st_crs(sb_protected_areas)$units
PA_buffer_5km <- sf::st_buffer(sb_protected_areas, dist = 5000)
...
sf::st_crs(goleta)$units
goleta_buffer_15km <- sf::st_buffer(goleta, dist = 15000)

goleta_PAs_within <- sf::st_within(sb_protected_areas, goleta_buffer_15km)
goleta_PAs_intersect <- sf::st_intersects(sb_protected_areas, goleta_buffer_15km)
goleta_PAs_intersection <- sf::st_intersection(sb_protected_areas, goleta_buffer_15km)

# Check class
class(goleta_PAs_intersect) == class(goleta_PAs_intersection)

# Distance-based join
goleta_PAs_join <- sf::st_join(sb_protected_areas, goleta, st_is_within_distance, dist = 15000)
