...
Spatial interpolation is a method used to estimate values at unobserved locations based on observed data from nearby points. It assumes Tobler's first law of geography, which states that everything is related to everything else, but near things are more related to each other. Two of the most popular spatial interpolation methods are Inverse Distance Weighting (IDW) and ordinary/universal Kriging. Today, IDW differs from the nearest neighbor approach by assigning less weight to points that are farther away.
