...
- Use `crop()` and `mask()` to create a subset of a raster or vector 
- Use `aggregate()` and `resample()` to create a new raster or vector
- Use `as.polygons()` to convert a raster to a vector of polygons
...
```{r}
zion_points <- sf::read_sf(system.file("vector/zion_points.gpkg", package = "spDataLarge"))
...
srtm_crop2 <- terra::crop(srtm, zion_convex_hull)
plot(srtm_crop1)
plot(srtm_crop2)

srtm_mask1 <- terra::mask(srtm, zion_points)
srtm_mask2 <- terra::mask(srtm, zion_convex_hull)
plot(srtm_mask1)
plot(srtm_mask2)
```
