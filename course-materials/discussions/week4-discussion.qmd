...
subtitle: "Practice with raster manipulations"
...
Refer to [Chapter 3](https://geocompr.robinlovelace.net/attr.html), [Chapter 4](https://geocompr.robinlovelace.net/spatial-operations.html), [Chapter 5](https://geocompr.robinlovelace.net/geometry-operations.html) of *Geocomputation with R* to get you
started.
:::
...
1. Plot a histogram and boxplot of the DEM raster of Zion National Park from `spDataLarge`
2. Reclassify the DEM raster and compute the mean for the three classes:
  - Low (less than 300)
  - Medium
  - High (greater than 500)
3. Calculate the Normalized Difference Water Index (NDWI) of a Landsat 8 image of Zion National Park
  - **Note**: $NDWI = (green - NIR)/(green + NIR)$
  a. Find a correlation between NDVI and NDWI
4. Find distances across all `peru_dem` cells to it's nearest coastline
  ...
  a. Weigh the distance raster with `peru_dem` and visualize the difference between the raster created using the Euclidean distance (E7) and the raster weighted by elevation
    - Every 100 altitudinal meters should increase the distance to the coast by 10 km
5. Change the resolution of the SRTM of the Zion National Park to 0.01 by 0.01 degrees
  - Use all of the method available in the `terra` package
  - **Note**: The `srtm` raster has a resolution of 0.00083 by 0.00083 degrees

## Answer Key

### Plot a histogram and boxplot of the DEM raster

```{r}
hist(dem)
```

## Reclassify elevation and find the mean
