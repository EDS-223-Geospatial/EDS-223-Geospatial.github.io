...
subtitle: "Generalizable functions"
author: "Ruth Oliver & Alessandra Vidal Meza"
date: last-modified
execute: 
  warning: false
  message: false
...
Santa Clara River — [The Nature Conservancy](https://www.nature.org/en-us/get-involved/how-to-help/places-we-protect/the-nature-conservancy-in-california-santa-clara-river-california-con/){target="_blank"}
:::
...
**Multi-spectral remote sensing data:** [Landsat's Operational Land Imager (OLI)](https://landsat.gsfc.nasa.gov/satellites/landsat-8/spacecraft-instruments/operational-land-imager/)
...
**Locations of representative vegetation communities:** Polygons representing sites

  - **study_site:** character string with plant type

Next, let's [download our data](https://drive.google.com/file/d/1szZrpzrWPSFnjE6U6nX8Zg3rRjeofXv9/view
). Unzip and move this to your version-controlled R Project's `data` folder. 
...
```{r }
#| echo: false
landsat_20180612 <- rast(here("course-materials", "data", "week8-section", "landsat_20180612.tif"))
```
...
### Compute NDVI for all scences

Now we want to repeat the same operations for all 8 scenes. Below is a solution (where we repeat each line for each scene), but it's pretty clunky.
...
#| echo: false
landsat_20180612 <-rast(here("course-materials", "data", "week8-section", "landsat_20180612.tif"))
landsat_20180815 <- rast(here("course-materials", "data", "week8-section", "landsat_20180815.tif"))
landsat_20181018 <- rast(here("course-materials", "data", "week8-section", "landsat_20181018.tif"))
landsat_20181103 <- rast(here("course-materials", "data", "week8-section", "landsat_20181103.tif"))
landsat_20190122 <- rast(here("course-materials", "data", "week8-section", "landsat_20190122.tif"))
landsat_20190223 <- rast(here("course-materials", "data", "week8-section", "landsat_20190223.tif"))
landsat_20190412 <- rast(here("course-materials", "data", "week8-section", "landsat_20190412.tif"))
landsat_20190701 <- rast(here("course-materials", "data", "week8-section", "landsat_20190701.tif"))
```
...
**If you've been following along, let's clear our environment `rm(list = ls()`and redefine our function for NDVI!**

## 2. Create function to compute NDVI

Let's define a function for NDVI, which computes the difference in reflectance between the **near infrared (NIR)** and **red** bands, normalized by their sum. 
...
#| echo: false
files <- list.files(
  here("course-materials", "data", "week8-section"), pattern = "*.tif$", 
  full.names = TRUE)
```
...
## 5. Update function to compute NDVI with list
...
## 6. Compute NDVI for each scene and create raster stack

Now we can use our function to create a NDVI layer for each scene and stack them into a single raster stack. And then update layer names to match date:
...
- Read shapefile of study sites to obtain data on vegetation communities
...
#| echo: false
#| output: false
sites <- st_read(here("course-materials", "data","week8-section","study_sites.shp"))
```
...
- Apply zonal statistics to obtain mean NDVI at study sites

Now we want to find the average NDVI within each study site. The output of `terra::zonal()` is a `data.frame` with rows that match the study site data set, so we bind the results to the original data set. 
