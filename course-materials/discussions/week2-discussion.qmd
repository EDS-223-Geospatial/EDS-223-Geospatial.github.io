...
1. Read in the data for Colombia's ecoregions, roads, and bird observations
  ...
  - Use `dplyr::rename()` or `dplyr::mutate()` to rename the columns `decimal_longitude` and `decimal_latitude` to `long` and `lat` in `aves` and `sf::st_as_sf()` to convert it into an sf object
...
4. Use `sf::st_crs()` to retrieve CRS of all vector objects and assign a new CRS
