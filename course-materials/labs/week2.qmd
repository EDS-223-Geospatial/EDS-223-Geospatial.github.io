...
# Source Materials
he following materials are modified from [Chapter 3 of Geocomputation with R by Rovin Lovelace](https://geocompr.robinlovelace.net/attr.html) and the [tmap book](https://r-tmap.github.io/tmap-book/).
:::
...
`sf`provides the same functionality that the `sp`, `rgdal`, and `rgeos` provided, but is more intuitive because it builds on the `tidyverse`. `sf` represents spatial objects as "simple feature" objects by storing them in a data frame with the geographic data stored in a special column (usually named `geom` or `geometry`).
...
Then, we supply some non-geographic attributes by creating a data frame with attributes about London. And we attach the simple feature collection and data frame to create a `sf` object. Check out the class of the new object we created.
...
# combine geomtery and data frame
